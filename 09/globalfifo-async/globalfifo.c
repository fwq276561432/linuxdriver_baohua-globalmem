...
//static int globalfifo_major = GLOBALFIFO_MAJOR; //lpq for dynamic
static int globalfifo_major = 0; //lpq for dynamic
/*globalfifo设备结构体*/
...
static int globalfifo_fasync(int fd, struct file *filp, int mode);//lpq
...
{
	/* 将文件从异步通知列表中删除 */
  globalfifo_fasync( - 1, filp, 0); // lpq
